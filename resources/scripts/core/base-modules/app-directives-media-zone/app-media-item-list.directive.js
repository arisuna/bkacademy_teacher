...
(function () {
    ...
    angular
        ...
        .directive('appMediaItemList', appMediaItemList);

    appMediaItemList.$inject = ['$http', '$window', '$timeout', '$location', 'AppMediaService', 'urlBase', 'WaitingService'];

    function appMediaItemList($http, $window, $timeout, $location, AppMediaService, urlBase, WaitingService) {
        return {
            restrict: 'E',
            replace: true,
            scope: {
                ...
                folders: '<?folders',
                assigneeFolders: '<?assigneeFolders',
                currentFolder: '<?currentFolder',
                currentAssigneeFolder: '<?currentAssigneeFolder',
            ...
            link: function (scope, element, attrs) {
                ...
                if (angular.isUndefined(scope.folderDspUuid) || scope.folderDspUuid == undefined) {
                    scope.folderDspUuid = "";
                }

                if (angular.isUndefined(scope.folderHrUuid) || scope.folderHrUuid == undefined) {
                    scope.folderHrUuid = "";
                }

                if (angular.isUndefined(scope.folderEeUuid) || scope.folderEeUuid == undefined) {
                    scope.folderEeUuid = "";
                }
            ...
            templateUrl: urlBase.tplBase('base-modules/app-directives-media-zone', 'media-item-list'),
