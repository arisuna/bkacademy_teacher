...
(function () {
    ...
    angular
        ...
        .directive('uploadZoneSession', uploadZoneSession);

    uploadZoneSession.$inject = ['FileUploader', '$http', '$localStorage', '$timeout', 'ngDialog', 'toaster', 'urlBase', '$translate', '$rootScope','$location','$window'];

    function uploadZoneSession(FileUploader, $http, $localStorage, $timeout, ngDialog, toaster, urlBase, $translate, $rootScope, $location, $window) {
        var uploadZoneBox = {
            restrict: 'E',
            replace: true,
            scope: {
            ...
            templateUrl: urlBase.tplBase('base-modules/uploader-with-session', 'upload-zone', '_=' + Math.random()),
                    ...
                    filters: [{
                        ...
                        'fn': function (item) {
                            if (item.size <= 10485760) {
                                ...
                                window.swal('Error', 'File too large', 'error');
                                return false;
                            }
                        }
                    }],
