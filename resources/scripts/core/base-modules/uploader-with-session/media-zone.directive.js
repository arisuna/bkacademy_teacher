(function () {
    ...
    angular
        ...
        .directive('mediaZoneSession', mediaZone);

    mediaZone.$inject = ['FileUploader', '$http', '$localStorage', '$timeout', 'ngDialog', 'toaster', 'urlBase', '$translate', '$rootScope','$location','$window','DataService','WaitingService'];

    function mediaZone(FileUploader, $http, $localStorage, $timeout, ngDialog, toaster, urlBase, $translate, $rootScope, $location, $window, DataService, WaitingService) {
        var uploadZoneBox = {
            restrict: 'E',
            replace: true,
            scope: {
            ...
            templateUrl: urlBase.tplBase('base-modules/uploader-with-session', 'media-zone', '_=' + Math.random()),
            ...
            controller: function ($scope, $element, $attrs, $translate, $location, $window) {
                ...
                $scope.getFileList = function(){
                    DataService.getMediaListByUuid( $scope.uuid ).then(function(res){
                        if(res.success == true){
                ...
                $scope.addItem = function (items) {
                    ...
                    if (!angular.isUndefined($scope.uuid) && $scope.uuid != '') {
                        $http({
                            ...
                            if (!res.success) {
                                WaitingService.error('Attach file was error');
                            } else {
                                //
                            }
                        });
                    ...
                    filters: [{
                        ...
                        'fn': function (item) {
                            if (item.size <= 10485760) {
                                ...
                                WaitingService.error('File too large');
                                return false;
                            }
                        }
                    }],
