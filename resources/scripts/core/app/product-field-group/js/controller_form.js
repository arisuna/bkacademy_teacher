...
(function () {
    ...
    App.controller('ProductFieldGroupFormController', ['$scope', '$http', '$stateParams', '$state', '$location', '$anchorScroll',
        'WaitingService', 'AppDataService', 'AppProductFieldGroupService', 'AppSystem',
        function ($scope, $http, $stateParams, $state, $location, $anchorScroll,
            ...
            $scope.getDetailFn = function () {
                ...
                AppProductFieldGroupService.detailProductFieldGroup(id).then(
                    function (res) {
                        if (res.success) {
                            if ($scope.isClone) {
                                res.data.id = 0;
            ...
            $scope.deleteFn = function (id) {
                WaitingService.questionSimple('DO_YOU_WANT_TO_DELETE_BUSINESS_ZONE_TEXT',
                    function (res) {
                        AppProductFieldGroupService.deleteProductFieldGroup(id).then(function (res) {
                            if (res.success) {
                                // WaitingService.success(res.message, function () {
                                //     $state.go('app.system-attribute.list');
                                // });
            ...
            $scope.addProductFieldGroupValueFn = function (key) {
