...
(function () {
    ...
    numberInput.$inject = ['urlBase', '$timeout'];

    function numberInput(urlBase, $timeout) {
        ...
        return {
            ...
            template: `<div>
                <div class="input-group">
                    <span class="input-group-addon"
                          ng-if="numberInput.hasPrefix()">
                        {{numberInput.prefix}}
                    </span>
                    <input class="form-control relo-bg-bright-gray"
                           ng-style="{'border-color':'transparent'}"
                           name="{{ realName }}"
                           type="text"
                           ng-model="model"
                           string-to-number=""
                           ...
                           data-parsley-errors-container="#error_{{ realName }}"
                           ng-blur="numberInput.onBlur()">
                    <span class="input-group-addon relo-bg-bright-gray" ng-if="numberInput.hasPostfix()">{{numberInput.postfix}}</span>
                    <span class="input-group-btn">
                        <button class="btn btn-flat relo-bg-bright-gray text-white"
                                ng-disabled="disabled"
                                type="button" ng-click="numberInput.inc()"><i
                            class="fa fa-plus" aria-hidden="true"></i></button>
                        <button class="btn btn-flat relo-bg-bright-gray text-white"
                                ng-disabled="disabled"
                                type="button"
                                ng-click="numberInput.dec()"><i
                            class="fa fa-minus" aria-hidden="true"></i></button>
                    </span>
                </div>
                <div id="error_{{ realName }}"></div>
            </div>
            `,
            ...
            link: function (scope) {
                scope.realName = "sequence_" + _.uniqueId();

                if (angular.isUndefined(scope.start) || scope.start == '') {
                    scope.start = 0;
                }
            },
            controller: ["$scope", function ($scope) {
                ...
                $scope.$watch("model", function (newValue, oldValue) {
                    if (oldValue != undefined && oldValue != null && newValue != oldValue && _.isNumber(newValue) && newValue >= 0) {
                        $scope.onChange();
                    }
                });
