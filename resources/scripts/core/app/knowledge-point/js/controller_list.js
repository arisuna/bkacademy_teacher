(function () {
    'use strict';
    App.controller('KnowledgePointListApiController', ['$scope', '$state', '$timeout', '$rootScope', '$translate',
        'WaitingService', 'AppKnowledgePointService', 'ngDialog', 'urlBase', 'AppFilterConfigService',
        function ($scope, $state, $timeout, $rootScope, $translate,
            ...
            $scope.loadList = function () {
                ...
                AppKnowledgePointService.getList($scope.params).then(function (res) {
                    if (res.success) {
            ...
            $scope.subscribe('apply_filter_config_KNOWLEDGE_POINT', function (filterConfigId) {
            ...
            $scope.subscribe('sort_by_column_and_order_KNOWLEDGE_POINT', function (data) {
                angular.element('.scroll-append').scrollTop(0);
            ...
            $scope.subscribe('text_search_KNOWLEDGE_POINT', function (data) {
                angular.element('.scroll-append').scrollTop(0);
            ...
            $rootScope.$on('knowledge_point_filter_update', function (event, data) {
                ...
                if (data.chapters && data.chapters.length) {
                    $scope.params.chapters = data.chapters
                } else {
                    $scope.params.chapters = []
                }
                if (data.topics && data.topics.length) {
                    $scope.params.topics = data.topics
                } else {
                    $scope.params.topics = []
                }
                $timeout(function () {
            ...
            $scope.deleteFn = function (item, index) {
                WaitingService.questionSimple('DO_YOU_WANT_TO_DELETE_KNOWLEDGE_POINT_TEXT', function () {
                    AppKnowledgePointService.deleteKnowledgePoint(item.uuid).then(function (res) {
                        if (res.success) {
            ...
            $scope.createFn = function(){
                ...
                $scope.createDialog = ngDialog.open({
                    template: urlBase.tplApp('app', 'knowledge-point', 'form-dialog', '_=' + Math.random()),
                    ...
                    controller: ['$scope', '$element', '$timeout', 'WaitingService', 'AppKnowledgePointService', '$state',
                    function ($scope, $element, $timeout, WaitingService, AppKnowledgePointService, $state) {
                        $scope.object = $scope.ngDialogData.knowledge_point;
            ...
            $scope.editFn = function(knowledge_point){
                console.log('knowledge_point', knowledge_point);
                    ...
                    controller: ['$scope', '$element', '$timeout', 'WaitingService', 'AppKnowledgePointService', '$state',
                        function ($scope, $element, $timeout, WaitingService, AppKnowledgePointService, $state) {
                            ...
                            $scope.getDetailFn = function () {
                                AppKnowledgePointService.detailKnowledgePoint($scope.object.uuid).then(
                                    function (res) {
                                        if (res.success) {
