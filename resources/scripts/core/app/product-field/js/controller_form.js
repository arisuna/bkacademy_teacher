...
(function () {
    ...
    App.controller('ProductFieldFormController', ['$scope', '$http', '$stateParams', '$state', '$location', '$anchorScroll',
        'WaitingService', 'AppDataService', 'AppProductFieldService', 'AppSystem',
        function ($scope, $http, $stateParams, $state, $location, $anchorScroll,
            ...
            $scope.getDetailFn = function () {
                ...
                AppProductFieldService.detailProductField(id).then(
                    function (res) {
                        if (res.success) {
                            if ($scope.isClone) {
                                res.data.id = 0;
            ...
            $scope.deleteFn = function (id) {
                WaitingService.questionSimple('DO_YOU_WANT_TO_DELETE_BUSINESS_ZONE_TEXT',
                    function (res) {
                        AppProductFieldService.deleteProductField(id).then(function (res) {
                            if (res.success) {
                                // WaitingService.success(res.message, function () {
                                //     $state.go('app.system-attribute.list');
                                // });
            ...
            $scope.addProductFieldValueFn = function (key) {
