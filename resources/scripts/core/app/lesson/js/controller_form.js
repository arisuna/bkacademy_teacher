        ...
        function ($scope, $timeout, $http, $stateParams, $state, ngDialog, urlBase, WaitingService, AppAddressService, AppSystem, AppAclService, AppLessonService) {
            ...
            $scope.changeStatus = (item, index) => {
                if(item.value == 2){
                    ...
                    console.log($scope.lesson.verification_status);

                    if (!$scope.lesson.id_number ) {
                        mess = 'ID_NUMBER_IS_REQUIRED_TEXT';
                        console.log($scope.lesson.statusSelected);
                        WaitingService.error(mess);
                        $scope.setTab(3);
                        $timeout(function () {
                            $scope.lesson.statusSelected = $scope.statuses.find(o => o.value == $scope.lesson.verification_status)
                        })
                        return;
                    }
                    ...
                    // $scope.lesson.verification_status = item.value;
                    $scope.upgradeToLvl2();
            ...
            $scope.upgradeToLvl2 = function () {
                $scope.saving = true;
                AppLessonService.upgradeToLvl2($scope.lesson).then(function (res) {
                    if (res.success) {
                        WaitingService.popSuccess(res.message);
                        $scope.lesson.lvl = 2;
                        $scope.lesson.verification_status = 2;
                        // $scope.lesson.statusSelected = $scope.statuses.find(o => o.value === $scope.lesson.verification_status);
                    } else {
                        WaitingService.error(res.message);
                    }
