...
(function () {
    ...
    App.controller('ExamTypeFormController', ['$scope', '$http', '$stateParams', '$state', '$location', '$anchorScroll',
        'WaitingService', 'AppDataService', 'AppExamTypeService', 'AppSystem',
        function ($scope, $http, $stateParams, $state, $location, $anchorScroll,
            ...
            $scope.getDetailFn = function () {
                ...
                AppExamTypeService.detailExamType(id).then(
                    function (res) {
                        if (res.success) {
                            if ($scope.isClone) {
                                res.data.id = 0;
            ...
            $scope.deleteFn = function (id) {
                WaitingService.questionSimple('DO_YOU_WANT_TO_DELETE_EXAM_TYPE_TEXT',
                    function (res) {
                        AppExamTypeService.deleteExamType(id).then(function (res) {
                            if (res.success) {
                                WaitingService.popSuccess(res.message);
