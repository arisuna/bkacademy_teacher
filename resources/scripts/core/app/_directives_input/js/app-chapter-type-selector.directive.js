(function () {
    ...
    angular
        ...
        .directive('appChapterTypeSelector', appChapterTypeSelector);

        appChapterTypeSelector.$inject = ['Utils', 'urlBase', 'ngDialog'];

    function appChapterTypeSelector(Utils, urlBase, ngDialog) {
        var directive = {
            restrict: 'E',
            replace: true,
            scope: {
            ...
            templateUrl: urlBase.tplApp('app', '_directives_input', 'app-chapter-type-selector-item'),

            link: function (scope, element, attrs) {

                if (angular.isUndefined(scope.label) || scope.label == '') {
                    scope.label = 'CHAPTER_TYPE_TEXT';
                }
                ...
                if (angular.isUndefined(scope.chapterType)) {
                    scope.chapterType = null;
                }

                scope.realName = "chapter_type_selector_" + _.uniqueId();
            },
            ...
            controller: function ($scope, $element, $attrs) {
                ...
                $scope.selectItem = function (selectedItem) {
                    $scope.chapterType = angular.copy(selectedItem);
                    if (typeof $scope.ngChange == 'function' && angular.isDefined($scope.ngChange)) {
                        $scope.ngChange();
                    }
                };
                ...
                $scope.openSearchDialog = function ($event) {
                    ...
                    let searchDialog = ngDialog.open({
                        template: urlBase.tplApp('app', '_directives_input', 'app-chapter-type-selector-search-dialog'),
