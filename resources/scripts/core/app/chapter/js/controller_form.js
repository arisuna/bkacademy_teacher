...
(function () {
    ...
    App.controller('ChapterFormController', ['$scope', '$http', '$stateParams', '$state', '$location', '$anchorScroll',
        'WaitingService', 'AppDataService', 'AppChapterService', 'AppSystem',
        function ($scope, $http, $stateParams, $state, $location, $anchorScroll,
            ...
            $scope.getDetailFn = function () {
                ...
                AppChapterService.detailChapter(id).then(
                    function (res) {
                        if (res.success) {
                            if ($scope.isClone) {
                                res.data.id = 0;
            ...
            $scope.deleteFn = function (id) {
                WaitingService.questionSimple('DO_YOU_WANT_TO_DELETE_CHAPTER_TEXT',
                    function (res) {
                        AppChapterService.deleteChapter(id).then(function (res) {
                            if (res.success) {
                                WaitingService.popSuccess(res.message);
